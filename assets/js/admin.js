...
/* ── ADMIN AUTH GUARD (SINGLE-FIRE) ─────────────────── */
// Fires ONCE. Redirects to login if not authenticated or not admin.
// Uses location.replace → no history entry → no back-button loop.
export function requireAdmin(cb) {
  const unsub = onAuthStateChanged(auth, async user => {
    unsub(); // ← unsubscribe immediately — prevents repeat triggers
    if (!user) { location.replace('login.html'); return; }
    try {
      const snap = await getDoc(doc(db, 'users', user.uid));
      const data = snap.data();
      if (!data || data.role !== 'admin') { location.replace('login.html'); return; }
      cb(user, data);
    } catch {
